...
transformed parameters {
  array[5] vector[5] ass; // association strengths between pairs of sounds and shapes 
  ass[1,] = rep_vector(0.2, 5); ass[2,] = rep_vector(0.2, 5); ass[3,] = rep_vector(0.2, 5); ass[4,] = rep_vector(0.2, 5); ass[5,] = rep_vector(0.2, 5);
  ...
  for (t in 2:n_trials) { // loop over trials
    ...
    if (hit[t-1] == 1) // if correct on t-1
      ass[stim[t-1], resp[t-1]] *= (1+lr); // update association strength of the stimuli and response pair
